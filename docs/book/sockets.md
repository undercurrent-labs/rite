...
ended up, for the same reason `@http.listen` prints its bound address.
...
back to the caller, which is not what you want here.
...
Anything else, such as a record or an int, is an error rather than a silent
stringification.
...
TCP is a stream, so a `recv` returns *up to* `max_bytes` (default `65536`), so a short
...
The block runs **once per accepted connection**, in its own task, so a slow connection
...
closed when the block returns**. That is the lifetime rule, and it is why there
