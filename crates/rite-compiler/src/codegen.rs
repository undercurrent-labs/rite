...
pub fn generate_from_ir(ir: &ProgramIr, source_path: &Path) -> Result<String, String> {
    ...
    // A generated crate is built with whatever RUSTFLAGS the user has; `-Dwarnings` is
    // common enough that an unused import must not be able to fail their build. `Key` is
    // only referenced when the program builds a record.
    out.push_str("#![allow(unused_imports, unused_mut, unused_variables)]\n");
