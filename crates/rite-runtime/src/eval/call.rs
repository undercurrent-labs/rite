...
impl<'a> Evaluator<'a> {
    ...
    pub(super) async fn call_native(
        ...
        match name {
            ...
            "flat_map" => self.builtin_flat_map(args).await,
            "partition" => self.builtin_partition(args).await,
            "take_while" => self.builtin_take_drop_while(args, true).await,
            "drop_while" => self.builtin_take_drop_while(args, false).await,
            "update" => self.builtin_update(args).await,
