...
**Modules merge into one flat scope.** `merge_exports_into_entry` copies every imported module's public functions into the entry AST, and `inject_dependencies` does the same for each module before it is resolved (which is what lets a module `use` another). Qualified access is name-mangling: `math.square` and `@math.square` become the global `math__square`, rewritten in `desugar` and validated in `resolve` — so a qualifier must be bound in both places or you get either a false "undefined name" or a runtime failure naming the mangled symbol. A local binding shadows a bare module name (both sites check the mangled global exists rather than trusting the import alone); the `@` form is never shadowed, and capability namespaces are reserved as qualifiers (`use fs` is E022) so `@fs.read` can never mean a module. The merge returns its modules' qualifiers and the injected copies' names (`MergedImports`); resolve and desugar honour those qualifiers only inside the copies, which is what keeps a module's imports out of the entry's namespace.
